...
- 本规格与前端的 `GeneratePortraitRequest` 和 `GenerateProgressResponse` 相对应
- 依赖 `ai-portrait-async-task` 规格中的任务管理
- 依赖 `ai-portrait-database` 规格中的数据库表设计
- 依赖 `ai-portrait-file-storage` 规格中的文件存储
