...
### Requirement: 现有游戏兼容性
原有的无类型游戏 SHALL 被支持向后兼容，使用默认类型标记。

#### Scenario: 遗留游戏标记
- **WHEN** 系统迁移现有的无类型游戏
- **THEN** 自动为其分配类型 "3d-legacy"（表示原始 3D 编辑器）

#### Scenario: 遗留游戏访问
- **WHEN** 用户访问类型为 "3d-legacy" 的游戏
- **THEN** 系统将其重定向到 3D 射击编辑器，显示警告信息"此游戏使用旧格式"

#### Scenario: 数据不可用字段处理
- **WHEN** 系统读取现有游戏数据，但缺少新增字段（如 type、metadata）
- **THEN** 使用默认值填充：type = "3d-legacy"，metadata = {}
